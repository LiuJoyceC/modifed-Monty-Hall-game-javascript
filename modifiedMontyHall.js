...
if (drawcard === 'yes') {
    ...
    if (luckycard) {
        while (doors.indexOf(secondchoice) === -1) { //will keep reprompting until player enters valid response
            if (secondchoice != '') {
                notvalid();
            };
...
for (var cheating = ''; ["yes","no"].indexOf(cheating) === -1;) { //will keep reprompting until player enters valid response
    if (cheating != '') {
        notvalid();
    };
    cheating = prompt("Now you must answer honestly. Did you cheat by peeking behind the doors beforehand? ('yes' or 'no')").toLowerCase();
};
...
switch(true) {
    ...
    case (!luckycard): //If player drew the lucky card and already has two doors chosen, then we won't let them switch
        ...
        for (var stayorswitch = ''; ["stay","switch"].indexOf(stayorswitch) === -1;) { //will keep reprompting until player enters valid response
            if (stayorswitch != '') {
                notvalid();
            };
